# ui/clients_tab.py
# Self-contained Clients tab (no imports from ui.run_tab to avoid circular imports).
import os, re, io
...
# ---------- Supabase bootstrap (self-contained) ----------
...
# ---------- Client row (inline icon buttons ▦ ✎ ⟳ ⌫) ----------
def _client_row_icons(name: str, norm: str, cid: int, active: bool):
    ...
    with col_tog:
        if st.button("⟳", key=f"tg_{cid}", help=("Deactivate" if active else "Activate")):
            # read current, then flip (defensive)
            try:
...
# ---------- Report renderer (with high-contrast date chip) ----------
def _render_client_report_view(client_display_name: str, client_norm: str):
    ...
    # Render with strong date chip (class .chip .chip-date from core/styles.py)
    items_html = []
    for r in rows_f:
