# ui/run_tab.py
# Homespotter → full address (with number) via aggressive coord & text extraction + reverse geocode.
...
def _extract_latlon_inline(html_txt: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    for pat in [
        ...
        r'<meta[^>]+itemprop=[\'"]latitude[\'"][^>]+content=[\'"]([\-0-9\.]+)[\'"]',
        r'<meta[^>]+itemprop=[\'"]longitude[\'"][^>]+content=[\'"]([\-0-9\.]+)[\'"]',
    ]:
        pass
...
def resolve_any_link_to_zillow_rb(source_url: str) -> Tuple[str, str, str]:
    ...
    if not deeplink:
        return target, ", ".join([p for p in [street, f"{city} {state}".strip(), zipc] if p]), "fallback"
