# ui/run_tab.py
# Address/Link resolver with: Homespotter address-first → Zillow, thumbnails,
# client dedupe/tours (Supabase), and a "Fix properties" link cleaner.
# Keeps render_run_tab(state) signature expected by app.py.
...
# --- Improve /homes → /homedetails canonicalization ---
def upgrade_to_homedetails_if_needed(url: str) -> str:
    ...
    try:
        ...
        # 2) Direct anchors to homedetails
...
# ---------- Permissive URL state check (NC default) ----------
def url_matches_city_state(url: str, city: Optional[str] = None, state: Optional[str] = None) -> bool:
    ...
    if state:
        ...
        if (f"-{st2}-" not in u) and (f"/{st2}/" not in u):
            return False
    if city:
        cs = re.sub(r'[^a-z0-9]+','-', city.lower().strip())
        if f"-{cs}-" not in u:
            return False
    return True
...
def _is_homespotter_like(u: str) -> bool:
    try:
        ...
        return ("l.hms.pt" in h) or ("idx.homespotter.com" in h) or ("homespotter.com" in h)
    except Exception:
        return False

def _first(html: str, pat: str) -> str:
    m = re.search(pat, html, re.I)
    return m.group(1).strip() if m else ""

def _extract_addr_homespotter(html: str) -> Dict[str, str]:
    """
    Robust address extractor for Homespotter/IDX pages.
    Returns dict: {"street","city","state","zip"} (values may be "").
    """
    out = {"street":"", "city":"", "state":"", "zip":""}
    if not html:
        return out

    # 1) JSON-LD common on IDX pages
    for blk in _jsonld_blocks(html):
        ...
        if isinstance(addr, dict):
            ...
            out["state"]  = out["state"]  or (addr.get("addressRegion", "") or addr.get("addressCountry",""))[:2]
            out["zip"]    = out["zip"]    or addr.get("postalCode", "")

    # 2) Inlined app state (Homespotter often ships these fields)
    if not out["street"]:
        line  = _first(html, r'"(?:line|address1|street|streetAddress)"\s*:\s*"([^"]+)"')
        city  = _first(html, r'"(?:city|addressLocality)"\s*:\s*"([^"]+)"')
        state = _first(html, r'"(?:state(?:_code|OrProvince)?|addressRegion|region)"\s*:\s*"([A-Za-z]{2})"')
        zipc  = _first(html, r'"(?:postal(?:_code|Code)?|zip)"\s*:\s*"(\d{5}(?:-\d{4})?)"')
        if line:  out["street"] = line
        if city:  out["city"]   = city
        if state: out["state"]  = state
        if zipc:  out["zip"]    = zipc

    # 3) Microdata itemprop fallbacks
    if not out["street"]:
        out["street"] = out["street"] or _first(html, r'itemprop=["\']streetAddress["\'][^>]*>\s*([^<]+)')
        out["city"]   = out["city"]   or _first(html, r'itemprop=["\']addressLocality["\'][^>]*>\s*([^<]+)')
        out["state"]  = out["state"]  or _first(html, r'itemprop=["\']addressRegion["\'][^>]*>\s*([A-Za-z]{2})')
        out["zip"]    = out["zip"]    or _first(html, r'itemprop=["\']postalCode["\'][^>]*>\s*(\d{5}(?:-\d{4})?)')

    # 4) og:title / <title> with full address (last resort)
    if not out["street"]:
        title = _first(html, r"<meta[^>]+property=['\"]og:title['\"][^>]+content=['\"]([^'\"]+)['\"]") or \
                _first(html, r"<title>\s*([^<]+?)\s*</title>")
        if title and re.search(r"\b[A-Za-z]{2}\b", title) and re.search(r"\d{5}", title):
            out["street"] = title  # we’ll split later if needed

    return out

def resolve_homespotter_to_zillow(src_url: str, *, preferred_state: str = "NC") -> Tuple[str, str]:
    """
    Address-first resolver: expand Homespotter link → parse address → search Zillow by address.
    Returns (zillow_url, composed_address_for_logs).
    """
    final_url, html, _ = expand_url_and_fetch_html(src_url)

    # If we hit a short link that redirected away, follow again to get the real IDX HTML
    if _is_homespotter_like(src_url) and not _is_homespotter_like(final_url):
        final_url, html, _ = expand_url_and_fetch_html(final_url)

    # Parse address from page
    addr = _extract_addr_homespotter(html)
    if preferred_state and not addr.get("state"):
        addr["state"] = preferred_state

    street = addr.get("street","").strip()
    city   = addr.get("city","").strip()
    state  = addr.get("state","").strip()
    zipc   = addr.get("zip","").strip()

    # If we failed to get a usable address, just return the expanded URL (do NOT emit nc_rb).
    if not (street and (city or zipc or state)):
        return final_url, ""

    # Try Bing → Zillow homedetails first (best quality result)
    variants = generate_address_variants(street, city, state or preferred_state, zipc, {"state": preferred_state})
    z2, _ = resolve_homedetails_with_bing_variants(
        variants, required_state=state or preferred_state, required_city=(city or None), mls_id=None, delay=0.3, require_match=False
    )
    if z2:
        return z2, compose_query_address(street, city, state or preferred_state, zipc, {"state": preferred_state})

    # Fall back: /homes RB slug, then attempt to upgrade to homedetails
    rb = construct_deeplink_from_parts(street, city, state or preferred_state, zipc, {"state": preferred_state})
    hd = upgrade_to_homedetails_if_needed(rb)
    return (hd or rb), compose_query_address(street, city, state or preferred_state, zipc, {"state": preferred_state})

# ---------- Resolve from arbitrary source URL (NC default) ----------
def resolve_from_source_url(source_url: str, defaults: Dict[str,str]) -> Tuple[str, str]:
    ...
    # Homespotter/IDX first — address-first (no MLS dependency)
    if _is_homespotter_like(source_url) or _is_homespotter_like(final_url):
        z, addr = resolve_homespotter_to_zillow(final_url or source_url, preferred_state=(defaults.get("state") or "NC"))
        return z, addr

    # Generic page → try HTML address → search Zillow by address
    ...
    # Fall back only if we have something usable
    ...
    # Nothing better — return expanded URL
    return final_url, ""

# ---------- Primary resolver ----------
def process_single_row(row, *, delay=0.5, land_mode=True, defaults=None,
                       require_state=True, mls_first=False, default_mls_name="", max_candidates=20):
    defaults = defaults or {"city":"", "state":"NC", "zip":""}
    ...
    query_address = variants[0] if variants else compose_query_address(street_raw, comp["city"], defaults.get("state",""), comp["zip"], defaults)
    deeplink = construct_deeplink_from_parts(street_raw, comp["city"], defaults.get("state",""), comp["zip"], defaults)
    ...
    # (We keep MLS helpers around, but you asked to prioritize address-first. Set mls_first=False by default.)
...
# ---------- Tours cross-check (optional; won't error if missing) ----------
...
def _process_rows(rows: List[Dict[str, Any]], *, land_mode=True, delay=0.4,
                  require_state=True, mls_first=False, default_mls_name="",
    ...
    for row in rows:
        ...
        if src_url:
            # NEW: this path will call the Homespotter address-first resolver when needed
            zurl, inferred_addr = resolve_from_source_url(src_url, defaults)
            preview = pick_canonical_url(zurl)
            results.append({
...
def render_run_tab(state: dict):
    ...
    with st.expander("Advanced"):
        ...
        require_state = st.checkbox("Require NC state match for Bing hits", value=False)
        # IMPORTANT: keep MLS off by default per your request (address-first)
        mls_first = st.checkbox("Try MLS lookup first", value=False)
    ...
    st.caption("Paste any Homespotter or Zillow links. I’ll output clean canonical **Zillow** URLs (address-first).")
    ...
    if st.button("🔧 Fix / Re-run links"):
        ...
        for i, u in enumerate(lines, start=1):
            best = u
            try:
                # If Homespotter, resolve by address → Zillow; else try normal resolver & upgrade
                if _is_homespotter_like(best):
                    z, _addr = resolve_homespotter_to_zillow(best, preferred_state="NC")
                    best = z or best
                else:
